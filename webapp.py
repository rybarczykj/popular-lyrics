...
def representsRange(r):
    try:
        ...
        if (
            len(dashSplit) == 2
            and representsInt(dashSplit[0])
            and representsInt(dashSplit[1])
        ):
            return dashSplit
        elif (
            len(commaSplit) == 2
            and representsInt(commaSplit[0])
            and representsInt(commaSplit[1])
        ):
            return commaSplit
        else:
            return []
...
def showYearData(year):
    ...
    if topSongs != None and topWords != None:
        ...
        return render_template(
            "year.html", year=year, topWords=topWords, topSongs=topSongs
        )
    else:
...
def showYearRangeData(yearRange):
    ...
    if topWords != None:
        ...
        os.replace("wordCloud.png", "static/wordCloud.png")
        return render_template(
            "yearrange.html", startYear=startYear, endYear=endYear, topWords=topWords
        )
    else:
