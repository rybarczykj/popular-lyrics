...
class DataSource:
    ...
    def getWordInstancesInSong(self, word, songID):
        ...
        try:
            ...
            query = (
                "SELECT Frequency FROM WORDCOUNT WHERE SongID = (%s) AND Word = (%s)"
            )
    ...
    def getWordInstancesInYear(self, word, year):
        ...
        try:
            if not isinstance(word, str) or (not isinstance(year, int)):
                ...
                raise ValueError(
                    "year should be within the range 1965-2015. The value of x was: {}".format(
                        year
                    )
                )
            else:
    ...
    def getUniqueWordInstancesInYear(self, word, year):
        ...
        try:
            if not isinstance(word, str) or (not isinstance(year, int)):
                ...
                if (
                    len(results) != 0
                ):  # fetchall returns a list, we only care about the first
                    return results[0][0]
                else:
                    return 0
        except Exception as e:
    ...
    def getTopXWordsofArtist(self, artistName, numberOfWords=50):
        ...
        try:
            cursor = self.connection.cursor()
            query = (
                "SELECT UNCOMMONWORDS.Word, SUM(UNCOMMONWORDS.Frequency) "
                "FROM "
    ...
    def getTopXWordsInYear(self, year, numberOfWords=100):
        ...
        try:
            if (year < 1965) or (year > 2015):
                ...
                cursor = self.connection.cursor()
                query = (
                    "SELECT YEARCOUNTS.Word, SUM(YEARCOUNTS.Frequency) AS Frequency "
    ...
    def getTopXSongsInYear(self, year, numberOfSongs=100):
        ...
        try:
            ...
            cursor = self.connection.cursor()
            query = (
                "SELECT SONG.Ranking, SONG.Artist, SONG.Name "
                "FROM SONG "
    ...
    def getTopXWordsInYearRange(self, year1, year2, numberOfWords=200):
        ...
        try:
            if year1 > year2:
                ...
                cursor = self.connection.cursor()
                query = (
                    "SELECT YearRangeCounts.Word, SUM(YearRangeCounts.Frequency) AS Frequency "
